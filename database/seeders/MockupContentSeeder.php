...
class MockupContentSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($commissioners as $i => [$n, $ri, $re, $img]) {
            Person::create(['name' => $n, 'role_id' => $ri, 'role_en' => $re, 'group' => 'commissioners', 'photo' => 'seed/board/' . $img, 'sort' => $i,
                'bio_id' => 'Profil singkat ' . $n . ' di Combiphar Group.', 'bio_en' => 'A brief profile of ' . $n . ' at Combiphar Group.']);
        }
        $directors = [
        ...
        foreach ($directors as $i => [$n, $ri, $re, $img]) {
            Person::create(['name' => $n, 'role_id' => $ri, 'role_en' => $re, 'group' => 'directors', 'photo' => 'seed/board/' . $img, 'sort' => $i,
                'bio_id' => 'Profil singkat ' . $n . ' di Combiphar Group.', 'bio_en' => 'A brief profile of ' . $n . ' at Combiphar Group.']);
        }
        ...
        foreach ($ms as $i => [$y, $ci, $ce, $img]) {
            Milestone::create(['year' => $y, 'caption_id' => $ci, 'caption_en' => $ce, 'photo' => $img, 'sort' => $i]);
        }

        // ---- Impact programs (with photos) ----
        DB::statement('SET FOREIGN_KEY_CHECKS=0');
        ImpactProgram::truncate();
        DB::statement('SET FOREIGN_KEY_CHECKS=1');
        $imp = [
        ...
        foreach ($imp as $i => [$ti, $te, $bi, $be, $img]) {
            ImpactProgram::create(['title_id' => $ti, 'title_en' => $te, 'body_id' => $bi, 'body_en' => $be, 'image' => $img, 'sort' => $i]);
        }

        // ---- Product category images ----
        ProductCategory::where('slug', 'consumer-health')->update(['image' => 'seed/product-consumer-health.png']);
        ProductCategory::where('slug', 'speciality-care')->update(['image' => 'seed/product-specialty-care.png']);
        ProductCategory::where('slug', 'nutrition-herbal')->update(['image' => 'seed/product-nutrition-herbal-care.png']);

        // ---- News cover images (health articles) ----
        $imgs = ['seed/news/piring-sahur.jpg', 'seed/news/zinc.jpg', 'seed/news/minuman-herbal.jpg'];
        foreach (Article::where('category', 'edukasi_gaya_hidup')->orderBy('id')->get() as $i => $a) {
            if (isset($imgs[$i])) $a->update(['cover_image' => $imgs[$i]]);
        }
        ...
        DB::statement('SET FOREIGN_KEY_CHECKS=1');
        for ($i = 1; $i <= 7; $i++) {
            Award::create(['title_id' => 'Penghargaan ' . $i, 'title_en' => 'Award ' . $i, 'image' => 'seed/awards/award-' . $i . '.png', 'sort' => $i]);
        }

        // ---- Global site images ----
        GlobalSite::where('name', 'Manufacturing Solutions')->update(['image' => 'seed/about/manufacturing.jpg']);
        GlobalSite::where('name', 'International Business')->update(['image' => 'seed/about/international.jpg']);
    }
}